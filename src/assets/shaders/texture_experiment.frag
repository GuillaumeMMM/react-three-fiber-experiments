...
vec4 addAmbiantNoise(vec2 pos, vec4 c) {
    ...
    float n = fract(sin(dot(pos, vec2(12.9898, 78.233))) * 
                    43758.5453);
    c *= (1.0 - noiseAmount + n * noiseAmount  + n * n * n * noiseAmount / 2.);
	return c;
}
...
void main(){
    ...
    gl_FragColor= addAmbiantNoise(Position.xy, vec4((vec3(color1F) * vec3(valueStep) + vec3(color2F) * (1. - vec3(valueStep))) * vec3(dProd), 1.));
}